...
- [8. Amortized time](#8-amortized-time)
...
### 4.4 Stack
Работает по принципу LIFO (Last In, First Out — последним пришел, первым ушел).

Добавление элемента $push(x)$: кладем элемент сверху. 

Удаление элемента $pop()$: забираем верхний элемент. 

Обе операции занимают $O(1)$. Поиск занимает $O(n)$ 

### 4.4 Queue
Работает по принципу FIFO (First In, First Out — первым пришел, первым ушел).

Bспользуют два указателя: head (голова, откуда забираем) и tail (хвост, куда добавляем).

Добавление элемента $push(x)$: пишем элемент в хвост.

Удаление эелемента $pop()$: забираем элемент головы 
...
## 8. Amortized time
Амортизированное время ($\tilde{T}$) это среднее время выполнения операции в расчете на длинную последовательность действий.

Например, если одна операция push была дорогой (пришлось копировать весь массив), остальные тысячи push были мгновенными. В сумме среднее время составит $O(1)$. Эта оценка дает более реалистичную картину производительности, чем Worst Case.
