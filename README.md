...
- [4. Data Structures](#4-data-structures)
  ...
  - [Stack](#44-stack)
  - [Queue](#45-queue)
  - [Linked List](#46-linked-list)
  - [Disjoint Set Union](#47-disjoint-set-union)
  - [Segment Tree](#48-segment-tree)
  - [Binary Search Tree](#49-binary-search-tree)
...
Используют два указателя: head (голова, откуда забираем) и tail (хвост, куда добавляем).
...
### 4.6 Linked List
В обычном массиве все элементы лежат в памяти плотной очередью. В связном списке элементы (узлы) могут быть разбросаны по всей оперативной памяти. Чтобы список не рассыпался, каждый узел хранит: **Value** (сами данные) и **Next** (указатель на следующий узел).

**Односвязный список**: Связь идет только в одну сторону: от головы (head) к хвосту.

**Двусвязный список**: В каждом узле добавляется второй указатель — prev (на предыдущий узел).

**Pointer Machine**: модель, в которой мы работаем с объектами через ссылки. Код `x = newNode()` создает новый узел, а `x.v = 5` записывает в него значение.

- Добавление и удаление элемента: разрезается связь между узлами, чтобы вставить/удалить новый узел между ними, далее нужно восстановить связь между элементами

Обе операции занимают $O(1)$. Но доступ по индексу `O(n)` (в массиве `O(1)`) 
<img width="800" height="330" alt="image_2026-03-20_160220660" src="https://github.com/user-attachments/assets/387c86f1-7abb-44c6-a87a-f859dde3bd39" />

### 4.7 Disjoint Set Union
Два множества называются непересекающимися, если у них нет общих элементов. Каждое множество представляется деревом; у каждого элемента есть родитель (parent); корень — элемент, который указывает сам на себя 

Она поддерживает следующие операции:

- Объединение (Union): Слияние двух непересекающихся множеств в одно.

- Поиск (Find): Поиск представителя (главного элемента) непересекающегося множества.

- Проверка принадлежности: Проверка того, принадлежат ли два элемента одному и тому же множеству или нет. Для этого мы находим представителей обоих элементов и проверяем, совпадают ли они.

Поиск может дойти до `O(n)`, что довольно долго. Способы оптимизации:

**Path Compression** ускоряет операцию `find()`: Идея в том, что когда мы ищем корень элемента, мы проходим по цепочке родителей вверх по дереву. После того как корень найден, все вершины на этом пути переподвешиваются напрямую к корню. В результате дерево становится более «плоским», и последующие вызовы find() выполняются значительно быстрее, потому что больше не нужно проходить через промежуточные узлы. Все последующие вызовы для этих элементов будут занимать `O(1)`.

**Union by Rank/Size** ускоряет операцию `union()`: Чтобы дерево не росло в высоту, при объединении нужно всегда подвешивать меньшее дерево к большему. Благодаря этому высота дерева никогда не превысит `O(\log n)`.

Если использовать эти две оптимизации одновременно, то время работы одной операции становится почти константным.


### 4.8 Segment Tree
Дерево отрезков позволяет быстро работать с интервалами массива. Незаменима, когда нужно постоянно изменять элементы массива и отвечать на какието запросы, все за `O(\log n)`.

Поддерживает любую ассоциативную операцию, к примеру:

- Сумма, произведение

- Найти мин и макс элемент, а также их количество

- НОД, НОК 

**Реализация запроса суммы на отрезке `sum(l, r, x, lx, rx)`:** Всего 3 возможных случая пересечения нашего запроса [l, r) и зоны ответственности вершины [lx, rx):

1) `if l >= rx || lx >= r: return 0`

Смысл: Отрезки вообще не пересекаются. Вершина отвечает за участок, который нам не нужен.

Действие: Возвращаем 0

2) `if lx >= l && rx <= r: return value(x)`

Смысл: Отрезок вершины целиком лежит внутри нашего запроса.

Действие: Вершина x уже хранит заранее посчитанную сумму на своем отрезке. Мы просто возвращаем её значение `value(x)`.

3) `if l >= lx && rx >= r`

Смысл: Отрезки пересекаются, значит часть нужных нам данных лежит в левом ребенке, а часть — в правом.

Действие: Мы делим отрезок вершины пополам: `m = (lx + rx) / 2`. Затем рекурсивно запускаем поиск в левом ребенке `sl = sum(l, r, 2x+1, lx, m)` и в правом `sr = sum(l, r, 2x+2, m, rx)`. И складываем их ответы: `return sl + sr`.

**Реализация запроса изменения элемента `set(i, v, x, lx, rx)`:**

Базовый случай: `if rx - lx == 1`. Мы дошли до листа дерева. Обновляем значение: `val(x) = v` и `return`.

Спуск вниз: Делим отрезок пополам `m = (lx + rx) / 2`. Смотрим, где находится нужный нам индекс i:

`if i < m`: он в левой половине, идем туда: `set(..., 2x+1, lx, m)`.

`else`: он в правой половине, идем туда: `set(..., 2x+2, m, rx)`.

Подъем: `val(x) = val(2x+1) + val(2x+2)`. Когда мы изменили значение в самом низу, нам нужно на обратном пути из рекурсии обновить значения всех родителей, чтобы они хранили правильную сумму.

### 4.9 Binary Search Tree
Древовидная структура данных, где каждый узел содержит уникальный ключ, левый потомок меньше родителя, а правый — больше. Обеспечивает быстрый поиск, вставку и удаление элементов в среднем `(O(\log n)`. Позволяет реализовать структуры данных `Set` (множество) и `Map` (словарь/ассоциативный массив).

В худшем случае операции `insert` (вставка) и `remove` (удаление) занимают `O(height)`

**AVL-Tree** - это самобалансирующееся дерево поиска. Разница высот левого и правого поддеревьев для любого узла не должна превышать 1: `|H_1 - H_2| \le 1`. Благодаря этому условию высота дерева всегда логарифмична: `height \le c \cdot \log n`. Это значит, что поиск, вставка и удаление всегда работают очень быстро — за `O(\log n)`.

Когда после вставки нового узла условие баланса `|H_1 - H_2| \le 1` нарушается, дерево чинят с помощью вращений LL-turn и RR-turn. По итогу узел `y` опускается, а его левый сын `x` становится новым корнем поддерева, при этом отношения больше-меньше сохраняются.
